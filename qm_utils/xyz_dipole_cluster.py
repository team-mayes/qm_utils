...
def test_clusters(pucker_filename_dict, hartree_dict, xyz_dir, ok_tol, ring_num_list, print_option='off'):
    ...
    puckering_dipole_dict = {}

    difference_list = []

    for pucker, pucker_file_list in pucker_filename_dict.items():
        pucker_count = 0
        print('The length of the number of puckers ({}) is: {}.\n'.format(pucker, len(pucker_file_list)))

        if len(pucker_file_list) == 1:
            compare_dict = {}
            compare_dict[pucker_file_list[0]] = 0
            puckering_dipole_dict[pucker_file_list[0]] = compare_dict
            difference_list.append(pucker_file_list[0] + '#' + str(0.000) + '#' + pucker_file_list[0])
            ...
            for num_file_main in range(0, len(pucker_file_list)-1):
                ...
                for num_file_compare in range(pucker_count, len(pucker_file_list)):
                    dipole_difference = abs(float(hartree_dict[pucker_file_list[num_file_main]][DIPOLE]) -
                                            float(hartree_dict[pucker_file_list[num_file_compare]][DIPOLE]))
                    compare_dict[pucker_file_list[num_file_compare]] = round(dipole_difference,3)
                    if print_status != 'off':
                        print(pucker_file_list[num_file_main], round(dipole_difference, 3), pucker_file_list[num_file_compare])

                    difference_list.append(pucker_file_list[num_file_main] + '#' + str(round(dipole_difference, 3)) + '#' + pucker_file_list[num_file_compare])

                puckering_dipole_dict[pucker_file_list[num_file_main]] = compare_dict


    pucker_grouping_dict = {}
    pucker_number_status = {}

    for row in difference_list:
        row_info = row.split('#')
        pucker_id = hartree_dict[row_info[0]][PUCKER]

        if hartree_dict[row_info[0]][PUCKER] == hartree_dict[row_info[2]][PUCKER]:
            pucker_id = hartree_dict[row_info[0]][PUCKER]
            pucker_number_status[pucker_id] =+ 1

        if float(row_info[1]) < 0.1:
            if row_info[0] in pucker_grouping_dict.keys():
                pucker_grouping_dict[row_info[0]] = [row_info[2]]
            else:
                pucker_grouping_dict[row_info[0]] = [row_info[0]]
        ...
        if raw_cluster_len == 1:
            ...
            for file_id in range(1, raw_cluster_len):
                ...
                for assigned_cluster_name in process_cluster_dict:
                    ...
                    dipole_difference = abs(float(hartree_dict[pucker_file_list[file_name]][DIPOLE]) -
                                            float(hartree_dict[pucker_file_list[process_cluster_dict[assigned_cluster_name][0]]][DIPOLE]))
                    if dipole_difference < 0.1:
    ...
    # for main_file_key in puckering_dipole_dict.keys():
    #     for compare_file_key, list in puckering_dipole_dict[main_file_key].items():
    #         if hartree_dict[main_file_key][PUCKER] != hartree_dict[compare_file_key][PUCKER]:
    #             print('Something is wrong...the puckers do not match')
    #         elif hartree_dict[main_file_key] == hartree_dict[compare_file_key]:
    #             print('Only one file for the pucker!')
    #         elif hartree_dict[main_file_key][PUCKER] == hartree_dict[compare_file_key][PUCKER]:
    #             pucker_id = hartree_dict[main_file_key][PUCKER]
    #             pucker_id_count = 0
                ...
                # puckering_dipole_dict[num_file_main][num_file_compare] = dipole_difference
                # print(puckering_dipole_dict)

                # if dipole_difference < 0.1:
                #
                #     process_cluster_dict[cluster_name].append(pucker_file_list[num_file_compare])
                #     print('---- MATCH ----')
                # else:
                #     print('---- DIFFERENT ----')
