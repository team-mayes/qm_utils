...
"""
The purpose of this script is to perform Boltzmann Weighting within the k-means centers during
algorithm to factor in the importance of energy on the center location.
...
def _spherical_kmeans_single_lloyd(X, energy, n_clusters, max_iter=300,
    ...
    for i in range(max_iter):
        ...
        if sp.issparse(X):
            ...
            #TODO: FACTOR IN THE ENERGIES INTO THIS SECTION
            centers_boltz = _centers_dense_boltz(X, energy, labels, n_clusters, distances)
...
def spherical_k_means(X, energy, n_clusters, init='k-means++', n_init=10,
    ...
    if n_jobs == 1:
        ...
        for it in range(n_init):
            ...
            labels, inertia, centers, n_iter_ = _spherical_kmeans_single_lloyd(
                X, energy, n_clusters, max_iter=max_iter, init=init, verbose=verbose,
...
def _centers_dense_boltz(X, energy, labels, n_clusters, distances):
...
class SphericalKMeans_boltz(KMeans):
    ...
    def fit(self, X, energy=None):
        ...
        self.cluster_centers_, self.labels_, self.inertia_, self.n_iter_ = \
            spherical_k_means(
                X, energy, n_clusters=self.n_clusters, init=self.init,
